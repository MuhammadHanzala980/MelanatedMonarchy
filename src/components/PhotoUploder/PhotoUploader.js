...
function PhotoUploader(props) {
  ...
  function save_img(compressedFile) {
    ...
    var num = d.toString();
    const uploadTask = storage.ref(`/images/${num + compressedFile.name}`).put(compressedFile);
    uploadTask.on(
      "state_changed",
      (snapShot) => {
            ...
            <div className="col-lg-12">
              {!imgUploded ? (
                ...
                <div className="btn-box text-center">
                  <button className="theme-btn border-0 button-success mt-4" onClick={handleValidatePhoto}>Save</button>
                </div>
              </>)}
            </div>
          </div>
